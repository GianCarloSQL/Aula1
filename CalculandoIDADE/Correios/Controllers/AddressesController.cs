...
namespace Correios.Controllers
{
    ...
    {
        private CorreioContext db = new CorreioContext();
        ...
        [HttpGet]
        [Route("api/adress/{cep}/info")]
        public IQueryable<Address> GetByCep(string cep, bool? isCep)
        {
            return isCep == true? db.Addresses.Where(x => x.Cep == cep) 
                : db.Addresses.Where(x => x.Bairro == cep);
        }
    }
}
