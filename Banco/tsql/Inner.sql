...
select sum(v.Valor) as 'Valor Total Vendas 2019' from Vendas v
...
select SUm(Valor) as 'Total', MONTH(DatInc) as 'Mes', Year(DatInc) as 'Ano' from Vendas

group by MONTH(Vendas.DatInc), YEAR(DatInc) 
order by  sum(Valor) desc;
...
select top 1 v.Valor as 'Venda', c.Modelo from Vendas v
inner join Carros c on c.Id = v.Carro
group by v.Valor, c.Modelo
